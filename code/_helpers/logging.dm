...
/proc/log_admin(text,level=SEVERITY_NOTICE,ckey="",admin_key="",ckey_target="")
	...
	if (config.log_admin)
		game_log("ADMIN", text)
	send_gelf_log(short_message=text, long_message="[time_stamp()]: [text]",level=level,category="ADMIN",additional_data=list("_ckey"=rhtml_decode(ckey),"_admin_key"=rhtml_decode(admin_key),"_ckey_target"=rhtml_decode(ckey_target)))
...
/proc/log_game(text, level = SEVERITY_NOTICE, ckey = "", admin_key = "", ckey_target = "")
	...
	send_gelf_log(
		...
		level = level,
		category = "GAME",
		additional_data = list("_ckey" = rhtml_decode(ckey), "_admin_key" = rhtml_decode(admin_key), "_target" = rhtml_decode(ckey_target))
	)
...
/proc/log_access(text, level = SEVERITY_NOTICE,ckey="")
	...
	send_gelf_log(short_message=text, long_message="[time_stamp()]: [text]",level=level,category="ACCESS",additional_data=list("_ckey"=rhtml_decode(ckey)))

/proc/log_say(text, level = SEVERITY_NOTICE, ckey = "")
	if (config.log_say)
		game_log("SAY", text)
	send_gelf_log(short_message=text, long_message="[time_stamp()]: [text]",level=level,category="SAY",additional_data=list("_ckey"=rhtml_decode(ckey)))

/proc/log_ooc(text, level = SEVERITY_NOTICE, ckey = "")
	if (config.log_ooc)
		game_log("OOC", text)
	send_gelf_log(short_message = text, long_message = "[time_stamp()]: [text]", level = level, category = "OOC", additional_data = list("_ckey" = rhtml_decode(ckey)))

/proc/log_whisper(text, level = SEVERITY_NOTICE, ckey = "")
	if (config.log_whisper)
		game_log("WHISPER", text)
	send_gelf_log(short_message = text, long_message = "[time_stamp()]: [text]", level = level, category = "WHISPER", additional_data = list("_ckey" = rhtml_decode(ckey)))

/proc/log_emote(text, level = SEVERITY_NOTICE, ckey = "")
	if (config.log_emote)
		game_log("EMOTE", text)
	send_gelf_log(short_message = text, long_message = "[time_stamp()]: [text]",level = level,category = "EMOTE", additional_data = list("_ckey" = rhtml_decode(ckey)))
...
/proc/log_attack(text, level = SEVERITY_NOTICE, ckey = "", ckey_target = "")
	...
	send_gelf_log(
		...
		level = level,
		category="ATTACK",
		additional_data = list("_ckey" = rhtml_decode(ckey), "_ckey_target" = rhtml_decode(ckey_target))
	)
...
/proc/log_ntirc(text, level = SEVERITY_NOTICE, ckey = "", conversation = "")
	...
	send_gelf_log(
		...
		level = level,
		category = "NTIRC",
		additional_data = list("_ckey" = rhtml_decode(ckey), "_ntirc_conversation" = rhtml_decode(conversation))
	)
